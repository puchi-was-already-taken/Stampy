...
begin
  ...
  begin
    Result := Format('%s%.2d:%.2d (� %s%.2d:%.2d)', [Sign, Trunc(TotalHours), Round(Frac(TotalHours)
          * 60),
        Sign, Trunc(AvgHours), Round(Frac(AvgHours) * 60)]);
  end
  else
  begin
    Result := Format('%s%.2d:%.2d (� %s%.2d:%.2d | %s%.2d:%.2d)', [Sign, Trunc(TotalHours),
...
ddd. dd.mm.yyyy | [hh]:nn                 | [hh]:nn
ddd. dd.mm.yyyy | [hh]:nn                 | [hh]:nn
Summary:        | [hh]:nn (�: [hh]:nn)  | [hh]:nn (�: [hh]:nn)
}
